    ...
    <div class="content-wrapper">
        <div class="container-xxl flex-grow-1 container-p-y">
            <h4 class="py-3 mb-4"><span class="text-muted fw-light"> {{ trans('drivers/drivers_trans.edit') }}
                    /</span>{{ trans('drivers/drivers_trans.edit driver') }} </h4>
            <div class="row">
                <!-- Basic Layout -->
                <div class="col-xl">
                    <div class="card mb-4">
                        <div class="card-header d-flex justify-content-between align-items-center">
                            <h5 class="mb-0">{{ trans('drivers/drivers_trans.edit driver') }} </h5>
                            <small class="text-muted float-end">{{ trans('drivers/drivers_trans.edit driver') }} </small>
                        </div>
                        <div class="card-body">
                            <form action="{{ route('admin.drivers.edit', $driver->user_id) }}" method="post">
                                @csrf
                                <div class="mb-3">
                                    <label class="form-label"
                                        for="basic-default-firstname">{{ trans('drivers/drivers_trans.first Name') }}
                                    </label>
                                    <input type="text" value="{{ old('first_name', $driver->first_name) }}"
                                ...
                                <div class="mb-3">
                                    ...
                                    <input type="text" value ="{{ old('last_name', $driver->last_name) }}"
                                    ...
                                    <div class="col-md-12 mb-4">
                                        <label for="admin-select" class="form-label">{{ trans('vendors/vendors_trans.Select vendor') }}</label>
                                        <select id="admin-select"
                                                ...
                                                @if($vendors)
                                                <option value="{{ trans('vendors/vendors_trans.Select vendor') }}" selected disabled>
                                                    {{ trans('vendors/vendors_trans.Select vendor') }}
                                                </option>

                                                </option>
                                                @foreach ($vendors as $vendor)
                                                    <option value="{{ $vendor->user_id }}" {{ $driver->vendor_id == $vendor->user_id ? 'selected' : '' }}>
                                                        {{ $vendor->first_name }}
                                                    </option>
                                                @endforeach
                                            @endif
                                ...
                                <div class="mb-3">
                                    ...
                                    <input type="text" id="basic-default-phone"
                                        ...
                                        placeholder="{{ trans('drivers/drivers_trans.Driver price') }}"
                                        name="driver_price" value="{{ old('driver_price', $driver->driver_price) }}" />
